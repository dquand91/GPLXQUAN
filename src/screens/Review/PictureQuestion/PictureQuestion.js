...
import {setCurrentPictureQuestion} from '../../../redux/pictureQuestion/actions';

const titleText = 'Câu hỏi sa hình';
...
const pictureQuestionList = listQuestion.filter((item, index) => {
  return index >= 70;
});
...
class PictureQuestion extends Component {
  constructor(props) {
    super(props);
    this.state = {
      questionList: pictureQuestionList,
      questionNumber: 0,
    };
    console.log('constructor', this.state.questionList[0]);
...
function mapStateToProps(state) {
  return {
    currentQuestion: state.PictureQuestionReducer.currentQuestion,
  };
}

const mapDispatchToProps = dispatch => ({
  setCurrentQuestion: question => dispatch(setCurrentPictureQuestion(question)),
});
